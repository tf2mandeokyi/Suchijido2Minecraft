...
public enum ElementDataType {
	...
	ElementDataType(String englishName, String layerNameHeader, TableColumns columns) {
		this(englishName, layerNameHeader, columns, 0);
	}



	ElementDataType(String englishName, String layerNameHeader, TableColumns columns, int priority) {
	...
	public static ElementDataType fromLayerName(String layerName) {
		if(layerName == null) return null;
		Matcher matcher = layerTypePattern.matcher(layerName);
		if(matcher.find()) {
			String match = matcher.group(1);
			for (ElementDataType t : values()) {
