...
import com.mndk.kmdi.core.dxfmap.DXFMapParser.Result;
...
public class FastDelaunayTriangulator {
	...
	public static FastDelaunayTriangulator from(List<Vector> points) {
		int n = points.size();
		Vector2dH[] coords = new Vector2dH[n];
		
		for(int i = 0; i < n; ++i) {
			coords[i] = new Vector2dH(points.get(i));
		}
		
		return new FastDelaunayTriangulator(coords);
	}
	...
	public TriangleList getTriangleList() {
		...
		for(int i = 0; i < this.triangles.length / 3; ++i) {
			result.add(new Triangle(this.coords[this.triangles[3 * i]], this.coords[this.triangles[3 * i + 1]], this.coords[this.triangles[3 * i + 2]]));
		}
		return result;
	}
	...
	public static void main(String[] args) throws IOException {
		...
		for(DXFMapContour contour : contours) {
			for(Vector2D vertex : contour.getVertexList()) {
				vertexList.add(vertex.toVector(contour.getElevation()));
			}
		}
		...
		for(int y=0;y<h;y++) {
			for(int x=0;x<h;x++) {
				double value = triangleList.interpolateY(new Vector2D(x, y));
